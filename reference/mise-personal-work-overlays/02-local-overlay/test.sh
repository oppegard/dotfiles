...
local_config="$fixture/mise.local.toml"
...
cp "$fixture/mise.local.personal.toml.example" "$local_config"
(cd "$test_root" && mise trust "$fixture/mise.toml")
(cd "$test_root" && mise trust "$local_config")
...
cp "$fixture/mise.local.work.toml.example" "$local_config"
(cd "$test_root" && mise trust "$local_config")
mise -C "$fixture" bootstrap dotfiles apply --yes
