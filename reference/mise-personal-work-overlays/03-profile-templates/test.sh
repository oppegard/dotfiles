...
(cd "$test_root" && mise trust "$fixture/mise.personal.toml")
(cd "$test_root" && mise trust "$fixture/mise.work.toml")

mise -C "$fixture" -E personal bootstrap dotfiles apply --yes
...
if grep -q 'company-docs' "$HOME/.codex/config.toml"; then
  echo "personal template unexpectedly contains work configuration" >&2
  exit 1
fi
